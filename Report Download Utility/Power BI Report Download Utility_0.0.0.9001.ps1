...
Specifies the name and path for the output file
...
Suffix for the filename of the output document.

Type: String
Options: None, Preview, Date
Default: None

.PARAMETER Suffix

Alias of OutputSuffix.

.PARAMETER WorkspaceName

The workspace name where the report resides.
...
Alias of PagesToInclude

.PARAMETER CredentialPath

Path to credentials file. GUI will allow you to save a
credentials file or sign-on manually.
...
# Version:        0.0.0.9001
# Author:         jay_sumners@discovery.com
...
# Set TLS security to pass credentials
...
if($requiresInteractive){
    ...
    ##### Build form #####
    # Add Objects
    $button_svc = New-Object System.Windows.Forms.Button -Property @{
        Font = $font_button_admin
        Text = "Admin"
        AutoSize = $false
        Width = $adj_width_admin_button
        Height = (Get-Y 20)
        Location = New-Object System.Drawing.Point (($credForm.Width - $marginX - $adj_width_admin_button - $h_space), $marginY)
    }
    ...
    $label_title = New-Object System.Windows.Forms.Label -Property @{
        ...
        Width = (Get-X 250)
        Height = (Get-Y 22)
        Location = New-Object System.Drawing.Point(($centerX - (Get-X (250/2))), ($button_svc.Location.Y + $button_svc.Height + $v_space_close))
    }
    ...
    function Switch_Enable_First {
        ...
        $t = @($button_usr_once, $button_usr_new, $button_usr_saved, $button_svc).Where({$_ -ne $passingButton})
    ...
    function Click_Admin {
        ...
        $button_svc.Enabled = $false

        # Main Operations
        $path = "\\us\shared\Global_Enterprise_Analytics\Microsoft_Power_BI\Administration"
        $credPath = ($path + "\Scripts\creds\cred_svcpowerbi_${env:USERNAME}_${env:COMPUTERNAME}.xml")

        $cred = Get-StoredCredentials -Path $credPath
        #$credForm.Hide()

        # Adjust form
        $null = Switch_Enable_First -passingButton $button_svc
        $null = Enable_Second
    ...
    $button_svc.Add_Click({Click_Admin -fromCredForm $true})
    ...
    $credForm.Controls.Add($button_svc)
    $credForm.Controls.Add($button_continue)

    ##### Show Form #####
    [void]$credForm.ShowDialog()
} 
elseif ($CredentialPath -ieq 'admin'){
    $path = "\\us\shared\Global_Enterprise_Analytics\Microsoft_Power_BI\Administration"
    $credPath = ($path + "\Scripts\creds\cred_svcpowerbi_${env:USERNAME}_${env:COMPUTERNAME}.xml")
...
if($OutputSuffix -ieq "preview"){
    $suffix = "_PREVIEW"
}
elseif($OutputSuffix -ieq "date"){
    $suffix = "_" + "$(Get-Date -Format "yyyy-MM-dd")"
}
else{
    $suffix = ""
}

$outfile = $outputDir + "\" + $report + $suffix + $ext
